...
class FusionClassifier(nn.Module):
    def __init__(self, num_points, input_demo_dim, latent_demo=32):
        super().__init__()
        self.sh_enc = PointNetfeat(global_feat=True, feature_transform=False)
        self.dm_enc = DemoEncoder(input_dim=input_demo_dim, latent_dim=latent_demo)
        self.joint = nn.Sequential(
            nn.Linear(1024 + latent_demo, 128),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 64),
            nn.ReLU(),
    ...
    def forward(self, shape_x, demo_x):
        z1, _, _ = self.sh_enc(shape_x.transpose(2, 1))  # z1 shape (B, 1024)
        z2 = self.dm_enc(demo_x)                         # z2 shape (B, latent_demo)
        z = torch.cat([z1, z2], dim=1)                   # (B, 1024 + latent_demo)
        out = self.joint(z)
...
model = FusionClassifier(num_points=num_points, input_demo_dim=input_demo_dim, latent_demo=32)
